...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    limited_account_refresh_batch_size: Number(config.limited_account_refresh_batch_size || 50),
...
type SettingsStore = {
  ...
  setLimitedAccountRefreshBatchSize: (value: string) => void;
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  saveConfig: async () => {
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        limited_account_refresh_batch_size: Math.max(1, Number(config.limited_account_refresh_batch_size) || 50),
  ...
  setLimitedAccountRefreshBatchSize: (value) => {
    set((state) => state.config ? { config: { ...state.config, limited_account_refresh_batch_size: value } } : {});
  },
